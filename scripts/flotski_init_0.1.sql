...
drop table if exists FLOTSKI_GUEST_TO_BOOKING;
...
create table FLOTSKI_GUEST_TO_BOOKING (
    ID           serial       PRIMARY KEY,
    GUEST_ID     integer,
    BOOKING_ID   integer,
    CONSTRAINT   "FK_USER_ID" FOREIGN KEY (GUEST_ID) REFERENCES FLOTSKI_GUEST(ID),
    CONSTRAINT   "FK_BOOKING_ID" FOREIGN KEY (BOOKING_ID) REFERENCES FLOTSKI_BOOKING(ID)
);

create UNIQUE INDEX UI_GUEST_TO_BOOKING_BOOKING_TO_GUEST ON FLOTSKI_GUEST_TO_BOOKING USING btree(BOOKING_ID, GUEST_ID);
